...
from srunner.extension.rl_integrate.macad_agent.agents.navigation.global_route_planner import (
    GlobalRoutePlanner,
)
from srunner.extension.rl_integrate.macad_agent.agents.navigation.local_planner import (
    LocalPlanner,
    RoadOption,
)
...
class BasicAgent(object):
    ...
    def get_transform_of_target(self):
        pass

    def _set_global_target(self, end_loc: carla.Location):
        self._base_target = end_loc

    def _get_emergency_target(self, distance: float = 10):
        wp, _ = self._local_planner.peak_second_waypoint_direction(index=0)
        if wp is None:
            return None
        the_wp = wp.next(distance)[0]
        if the_wp is None:
            return None
        the_loc = the_wp.get_right_lane().transform.location
        if the_loc is None:
            the_loc = the_wp.get_left_lane().transform.location
        return the_loc

    def _emergency_replan(self, distance: float = 20):
        if self._temp_target is None:
            loc = self._get_emergency_target(distance)
            if loc is None:
                return
            self.set_destination(loc)
            self._temp_target = loc
        else:
            if self.done():
                self._temp_target = None
                if self._base_target is not None:
                    self.set_destination(self._base_target)

    def _extra_do_step(
        self, actor_of_interest: "list[carla.Vehicle]" = None, verbose: bool = True
    ):
        """Execute one step of navigation. Only consider vehicle obstacle."""
        hazard_detected = False
        msg = ""

        # Retrieve all relevant actors
        if actor_of_interest is not None:
            id_list = [v.id for v in actor_of_interest]
            vehicle_list = actor_of_interest
            lights_list = []
        else:
            actor_list = self._world.get_actors()
            vehicle_list = actor_list.filter("*vehicle*")
        ...
        max_vehicle_distance = self._base_vehicle_threshold + vehicle_speed
        affected_by_vehicle, _, _ = self._vehicle_obstacle_detected(
            vehicle_list, max_vehicle_distance, id_list=id_list
        )
        if affected_by_vehicle:
            hazard_detected = True
            msg = "vehicle obstacle"
        ...
        if affected_by_vehicle:
            self._emergency_replan()

        control = self._local_planner.run_step()
        if hazard_detected:
            # control = self.add_emergency_stop(control)
            msg = "vehicle emergency_stop, I will not stop"

        if verbose:
            print(f"BasicAgent.extra_do_step: hazard={hazard_detected}, msg={msg}")

        return control

    def _extra_set_global_plan(
        self,
        plan: "list[tuple[carla.Waypoint, RoadOption]]",
        stop_waypoint_creation: bool = True,
        clean_queue: bool = True,
