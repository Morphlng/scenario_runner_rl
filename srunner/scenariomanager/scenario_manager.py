...
try:
    RL_AVAILABLE = True
    from srunner.extension.rl_integrate.macad_agent.rl_agent import RlAgent
except ImportError:
    import logging
    logging.warning("RL extension unavailable")
    RL_AVAILABLE = False
...
class ScenarioManager(object):
    ...
    def load_scenario(self, scenario, agent=None):
        ...
        # Register actor in scenario to RlAgentData
        if (
            RL_AVAILABLE
            and py_trees.blackboard.Blackboard().get("rl_actors") is not None
        ):
    ...
    def _check_rl_heterogenous(self):
        """
        Check if the scenario contains heterogenous agents
        """
        blackboard = py_trees.blackboard.Blackboard()
        rl_agents = blackboard.get("rl_agents")

        spaces = {}
        for agent in rl_agents.values():
            action_type = agent.agent_action.action_type
            if action_type not in spaces and "pseudo" not in action_type:
                spaces[action_type] = agent.agent_action.space_type

        return len(spaces) > 1

    def _enable_rl_action_padding(self):
        """
        Enable action padding for heterogenous agents
        """
        blackboard = py_trees.blackboard.Blackboard()
        rl_agents = blackboard.get("rl_agents")

        valid_actions = {
            actor_id: agent.agent_action.get_valid_actions()
            for actor_id, agent in rl_agents.items()
        }
        max_action_space = max(valid_actions.values(), key=len)
        padding_size = len(max_action_space)

        for agent in rl_agents.values():
            agent.agent_action.enable_action_padding(padding_size)
